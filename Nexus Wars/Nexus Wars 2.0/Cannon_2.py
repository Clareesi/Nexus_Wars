...
class Cannon(Game_piece):
    ...
    def draw(self, image):
        if self.activation:
            ...
            pygame.draw.circle(image, self.range_color, (self.x+(self.size/2), self.y+(self.size/2)), self.radius)                 
    ...
    def in_range(self, target):
        if self.activation and target.activation:
            return (abs(target.x-(self.x+(self.size/2)))**2+ abs((self.y+(self.size/2)-(target.y+target.size))**2) <= self.radius**2 or
                    abs((self.x+(self.size/2))-(target.x+target.size))**2 + abs((self.y+(self.size/2)) - (target.y+target.size))**2 <= self.radius**2 or
                    abs(target.x-(self.x+(self.size/2)))**2 + abs(target.y-(self.y+(self.size/2)))**2 <= self.radius**2 or
                    abs((self.x+(self.size/2))-(target.x+target.size))**2+ abs(target.y-(self.y+(self.size/2)))**2 <= self.radius**2)    
    ...
    def calculate_lead(self, target):
        ...
        b = 2 * target_vel[0] * (target_loc[0] - tower_loc[0]) + target_vel[1] * (target_loc[1] - tower_loc[1])
        c = (target_loc[0] - tower_loc[0])**2 + (target_loc[1] - tower_loc[1])**2
