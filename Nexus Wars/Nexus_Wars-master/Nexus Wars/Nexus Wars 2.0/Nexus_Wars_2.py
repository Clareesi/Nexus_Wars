...
class Nexus_wars(PygameHelper):
    ...
    def __init__(self, state="play"):
        ...
        PygameHelper.__init__(
            self, size=(self.w, self.h), fill=(self.bkg_color))
        ...
        self.e_building.activate(self.w - self.e_building.size, 150,
                                 pygame.Color("red"), self)
        self.p_cannon = Cannon()
        self.e_cannon = Cannon()
        self.e_cannon.activate(
            random.randrange((self.p_building.x + self.p_building.size +
                              self.e_cannon.radius),
                             (self.e_building.x - self.e_cannon.radius)),
            random.randrange(self.h), self.e_building,
            pygame.Color("light pink"))
    ...
    def mouseUp(self, button, pos):
        if not self.p_cannon.activation:
            if pygame.mouse.get_pressed():
                self.p_cannon.activate(pygame.mouse.get_pos()[0],
                                       pygame.mouse.get_pos()[1],
                                       self.p_building,
                                       pygame.Color("light green"))
    ...
    def draw(self):
        if self.state == "play":
            ...
            self.screen.blit(self.label,
                             (center(self.w, self.label.get_width()), center(
